...
def fomenki_parser_detail():
    print('Загрузка данных из файла JSON.')
    with open('app/storage/parser_events_data.json', 'r', encoding='utf-8') as jsonfile:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if title_tag:
                    ...
                    if hall_text == "Новая сцена, Большой зал":
                        hall_code = "HB-1"
                    elif hall_text == "Новая сцена, Малый зал":
                        hall_code = "HM-1"
                    elif hall_text == "Старая сцена, Зелёный зал":
                        hall_code = "CZ-2"
                    elif hall_text == "Старая сцена, Серый зал":
                        hall_code = "CC-1"
                    elif hall_text == "Новая сцена, Фойе":
                        hall_code = "HF"
    ...
    with open('app/storage/parser_full_event_data.json', 'w', encoding='utf-8') as jsonfile:
